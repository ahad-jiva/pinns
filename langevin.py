...
# langevin/fokker-planck equation for determining probability of a particle at (x, t)
...
N_data = 15
x_data = torch.rand(N_data, 1) * 8 - 4 # rescaled to [-4, 4]
...
# initial conditions: t = 0, random x, target is gaussian
N_ic = 100
x_ic = torch.randn(N_ic, 1) * 1.0 + 2.0 # sample close to gaussian mean
x_ic = x_ic.clamp(-4, 4) # just in case
t_ic = torch.zeros(N_ic, 1)
u_ic = true_u(x_ic, torch.zeros_like(x_ic))

# boundary conditions: x = -4, x = 4, random t
N_bc = 100
t_bc = torch.rand(N_bc, 1)
x_left = torch.ones(N_bc, 1) * -4
x_right = torch.ones(N_bc, 1) * 4
...
for epoch in tqdm(range(epochs)):
    ...
    x_col = torch.rand(N, 1, requires_grad=True) * 8 - 4
    ...
    # drift term and residual
    drift = (1.0 / 1.0) * (u_pred_col + x_col * u_x)
    diffusion = 0.1 * u_xx
    residual = u_t - drift - diffusion
    ...
    loss_pinn = loss_col + 50*loss_ic + loss_bc
    loss_pinn.backward()
    pinn_opt.step()
